...
pub struct OrganizationSnapshot {
    ...
    active_members: HashMap<String, String>,
    pending_invitations: HashMap<i64, PendingInvitation>,
    accepted_invitations: HashMap<i64, AcceptedInvitation>,
}
...
impl OrganizationSnapshot {
    pub fn active_member_username(&self, github_user_id: &str) -> Option<&str> {
        self.active_members.get(github_user_id).map(String::as_str)
    }

    pub fn pending_invitation(&self, invitation_id: i64) -> Option<&PendingInvitation> {
        self.pending_invitations.get(&invitation_id)
    }

    pub fn accepted_invitation(&self, invitation_id: i64) -> Option<&AcceptedInvitation> {
...
pub fn client(token: &str) -> Result<Client, Box<dyn Error>> {
    Ok(Client::builder()
        .user_agent("ainigma-github-poller/0.1")
...
pub async fn organization_snapshot(
    ...
    api_url: &str,
    organization: &str,
    unresolved_invitation_ids: &HashSet<i64>,
    ...
    Ok(OrganizationSnapshot {
        ...
        accepted_invitations: audit_log
            ...
            .filter_map(|entry| {
                Some((
                    entry.invitation_id?.as_i64()?,
                    AcceptedInvitation {
                        github_user_id: entry.user_id?.as_string(),
                    },
                ))
            })
            .collect(),
...
async fn organization_id(
    ...
    if response.status() == StatusCode::FORBIDDEN && sso_required(&response) {
        return Err(SnapshotError::SsoRequired);
    }
    if !response.status().is_success() {
    ...
    Ok(response.json::<Organization>().await?.id)
}
...
async fn list_member_additions(
    ...
    while let Some(url) = next_url.take() {
        ...
        for entry in page_entries {
            let invitation_id = entry
                .invitation_id
                .as_ref()
                .and_then(NumericOrString::as_i64);
...
async fn list_pending_invitations(
    ...
    loop {
        ...
        let page_invitations: Vec<PendingInvitation> = response.json().await?;
...
pub async fn find_pending_invitation(
    ...
    let invitations = list_pending_invitations(github, api_url, organization)
        ...
        .map_err(|error| match error {
            SnapshotError::SsoRequired => "github_sso_required".to_owned(),
...
#[cfg(test)]
mod tests {
    use super::{PendingInvitation, next_link};

    #[test]
    fn decodes_pending_organization_invitation_shape() {
        let invitation: PendingInvitation = serde_json::from_str(

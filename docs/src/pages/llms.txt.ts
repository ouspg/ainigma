...
export async function GET() {
  ...
  const lines = [
    ...
    "",
    `Full corpus (all pages, one document): ${new URL(withDocsBase("/llms-full.txt", docsVersions.current.base), config.site).href}`,
    "",
    "## Pages",
    "",
  ];
  ...
  for (const leaf of leaves) {
    ...
    rows.push({
      key: withDocsBase(leaf.url, docsVersions.current.base),
      line: `- [${leaf.title}](${new URL(withDocsBase(leaf.markdownUrl, docsVersions.current.base), config.site).href})${description}`,
    });
  }
  ...
  for (const group of groups) {
    ...
    rows.push({
      key: withDocsBase(`/${group.slug}`, docsVersions.current.base),
      line: `- [${group.label}](${new URL(withDocsBase(`/${group.slug}/llms.txt`, docsVersions.current.base), config.site).href})`,
    });
  }

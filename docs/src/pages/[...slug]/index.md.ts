...
export async function GET({ props }: { props: SlugProps }) {
  ...
  const body = [
    ...
    "",
    "> Documentation Index",
    `> Fetch the complete documentation index at: ${new URL(withDocsBase("/llms.txt", docsVersions.current.base), config.site).href}`,
    ...
    `Source: ${new URL(withDocsBase(sourceUrl ?? markdownUrl, docsVersions.current.base), config.site).href}`,
    "",
  ].join("\n");
